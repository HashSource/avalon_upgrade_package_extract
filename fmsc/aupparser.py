...
class AupHeader(KaitaiStruct):
    ...
    def _read(self):
        self.magic = (self._io.read_bytes(16)).decode("UTF-8")
...
class Aup2(KaitaiStruct):
    ...
    def _read(self):
        ...
        self.firmware_ver = (self._io.read_bytes(64)).decode("UTF-8")
        ...
        for i in range(self.hw_list_count):
            self.hw_list[i] = Fixed32Str(self._io, self, self._root)

        self.sw_list = [None] * self.sw_list_count
        for i in range(self.sw_list_count):
            self.sw_list[i] = Fixed32Str(self._io, self, self._root)
...
class Aup1(KaitaiStruct):
    ...
    def _read(self):
        ...
        _raw_hw_list = self._io.read_bytes(128)
        io = KaitaiStream(BytesIO(_raw_hw_list))
...
class CommaSeperatedStrList(KaitaiStruct):
    ...
    def _read(self):
        ...
        while not self._io.is_eof():
            self.hw_str_list.append(
                (self._io.read_bytes_term(44, False, True, False)).decode("UTF-8")
            )
            i += 1
