...
class AUPHeaderInfo(object):
    ...
    def firmware_ver(self):
        return (
            uf.to_str(self.parsed_header.header_data.firmware_ver).strip("\0").strip()
        )
    ...
    def all_supported_swtype_list(self):
        if self.is_legal() and hasattr(self.parsed_header.header_data, "sw_list"):
            return [
                uf.to_str(sw_item.str_value).strip("\0").strip()
    ...
    def total_len(self):
        ...
        if aup_ver == 0:
            ...
            return (
                ...
                + 32
                * (
                    self.parsed_header.header_data.hw_list_count
                    + self.parsed_header.header_data.sw_list_count
                )
                + 4
            )
...
class AUPFile(object):
    ...
    def generate_aup_header_payload(self, target_aup_ver):
        ...
        if target_aup_ver == 0:
            ...
            aup2_header_struct_before_crc = Struct(
                "<16sII64sIII%ss%ss"
                % (
                    len(hw_list) * fixed_list_item_len,
                    len(sw_list) * fixed_list_item_len,
                )
            )
            aup2_header_before_crc = aup2_header_struct_before_crc.pack(
                ...
                len(sw_list),
                uf.to_bytes(
                    "".join([hw.ljust(fixed_list_item_len, "\x00") for hw in hw_list])
                ),
                uf.to_bytes(
                    "".join([sw.ljust(fixed_list_item_len, "\x00") for sw in sw_list])
                ),
            )
            aup2_header_crc = struct.pack(
                "<I", (binascii.crc32(aup2_header_before_crc) & 0xFFFFFFFF)
            )
